...
export default function TransactionsPage() {
  ...
  const { data, isLoading, error } = useTransactions(page, limit, search);
            ...
            <table className="w-full text-left text-sm text-gray-400">
              ...
              <tbody className="divide-y divide-[#222]">
                {data?.data?.map((tx: any) => (
                  <tr key={tx.transactionId} className="hover:bg-[#1A1A1A] transition-colors">
                    <td className="px-6 py-4 font-medium text-[#E5E5E5]">{tx.voucherNumber}</td>
                    <td className="px-6 py-4">{format(new Date(tx.date), 'PP')}</td>
                    <td className="px-6 py-4">{tx.type}</td>
                    <td className="px-6 py-4">{tx.accountingDecision.ledger}</td>
                    <td className="px-6 py-4">
                      <span className={`px-2 py-1 rounded text-xs ${tx.voucher.status === 'SYNCED' ? 'bg-emerald-500/20 text-emerald-400' : 'bg-amber-500/20 text-amber-400'}`}>
                        {tx.voucher.status}
                      </span>
                    </td>
                    <td className="px-6 py-4 text-right">
                      <Link href={`/transactions/${tx.transactionId}`} className="text-blue-400 hover:text-blue-300 transition-colors">
                        View Details
                      </Link>
                    </td>
                  </tr>
