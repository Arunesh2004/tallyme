...
export class EventWorker {
  ...
  async processJob(job: any): Promise<void> {
    ...
    await CorrelationContext.run(contextData, async () => {
      this.logger.info(`Processing job ${job.id} for event ${envelope.metadata?.eventType}`);
      try {
...
export class WorkerRegistry {
  private workers: any[] = [];
  register(worker: any): void { this.workers.push(worker); }
  getAll(): any[] { return this.workers; }
}
