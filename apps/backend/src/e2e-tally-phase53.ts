  ...
  try {
    ...
    const candidate = await prisma.voucherCandidate.create({
      data: {
        ...
        entries: {
          create: [
            { sequence: 1, ledgerName: 'TallyMe_E2E_Ledger', isDebit: true, amount: 500, isParty: true },
            { sequence: 2, ledgerName: 'Cash', isDebit: false, amount: 500, isParty: false }
          ]
        }
      }
    });
    ...
    const syncResult = await erpEngine.syncVoucher(mappedData, 'TALLY_PRIME_V1', { 
    ...
    const validationRuns = await prisma.tallyValidationRun.findMany({ where: { companyId: company.id } });
