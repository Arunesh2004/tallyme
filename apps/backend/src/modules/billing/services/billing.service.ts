...
export class BillingService {
  ...
  async enforceUsageQuota(
    organizationId: string,
    featureFlag: string,
    metricType: string,
  ) {
    this.logger.log(
      `Enforcing usage quota for org ${organizationId} on ${featureFlag}`,
    );
    ...
    if (flag && !flag.isEnabled) {
      throw new ForbiddenException(
        `Feature ${featureFlag} is disabled for your organization.`,
      );
    }
    ...
    if (maxLimit > 0 && currentUsage >= maxLimit) {
      throw new ForbiddenException(
        `Usage limit reached for ${metricType}. Please upgrade your plan.`,
      );
    }
