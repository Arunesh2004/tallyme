...
import { IVoucherCandidateRepository, VoucherCandidate } from '../../domain/repositories';
...
export class PrismaVoucherCandidateRepository implements IVoucherCandidateRepository {
  ...
  async saveBalancedVoucher(voucher: VoucherCandidate, tx: ITransactionContext): Promise<void> {
    try {
      ...
      if (e.code === 'P2002') throw new InfrastructureException('Duplicate voucher', e);
      throw new InfrastructureException('Database error', e);
    }
  }

  async findPendingERPSync(): Promise<VoucherCandidate[]> { return []; }
  async markVoucherAsSynced(id: string, erpRef: string, tx: ITransactionContext): Promise<void> {}
  async markVoucherAsFailed(id: string, reason: string, tx: ITransactionContext): Promise<void> {}
}
