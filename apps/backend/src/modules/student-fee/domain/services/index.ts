...
import { IStudentRepository, IOutstandingFeeRepository } from '../../fee-automation/domain/repositories'; // From Module 5
...
export class StudentMatcher {
  ...
  async match(candidate: PaymentCandidate): Promise<Result<string, string>> {
    ...
    const student = await this.studentRepo.findStudentByEnrollmentNumber('STUB_ROLL');
    if (!student) {
...
export class FeeValidationPolicy {
  validate(candidate: PaymentCandidate, studentId: string): Result<boolean, string> {
    if (candidate.amount.amount.toNumber() <= 0) return fail('Amount must be positive');
