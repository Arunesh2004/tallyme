...
export class PaymentReference extends ValueObject<{ value: string }> {
  constructor(value: string) {
    if (!value || value.trim().length === 0) throw new ValidationException('PaymentReference cannot be empty');
...
export class TransactionId extends ValueObject<{ value: string }> {
  constructor(value: string) {
    if (!value || value.trim().length === 0) throw new ValidationException('TransactionId cannot be empty');
    super({ value: value.trim() });
  }
  get value(): string { return this.props.value; }
}
...
export class StudentRollNumber extends ValueObject<{ value: string }> {
  constructor(value: string) {
    if (!value || value.trim().length === 0) throw new ValidationException('RollNumber cannot be empty');
...
export class PaymentAmount extends ValueObject<{ amount: IDecimal }> {
  constructor(amount: IDecimal | number) {
    ...
    if (wrappedAmount.toNumber() <= 0) throw new ValidationException('PaymentAmount must be greater than zero');
