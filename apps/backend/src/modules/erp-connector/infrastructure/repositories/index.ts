...
export class PrismaERPSyncJobRepository implements IERPSyncJobRepository {
  ...
  async registerSyncAttempt(job: ERPSyncJob, tx?: ITransactionContext): Promise<void> {
    try {
  ...
  async updateSyncStatus(jobId: string, status: string, tx: ITransactionContext): Promise<void> {}
